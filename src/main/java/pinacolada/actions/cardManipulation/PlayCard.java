...
public class PlayCard extends EYBActionWithCallbackT2<AbstractMonster, AbstractCard>
{
    ...
    public PlayCard SetSourcePile(CardGroup sourcePile)
    {
        ...
        if (card != null)
        {
            PCLGameUtilities.TrySetPosition(sourcePile, card);
        }
    ...
    protected void UpdateInternal(float deltaTime)
    {
        ...
        {
            if (PCLGameUtilities.RequiresTarget(card) && (target == null || PCLGameUtilities.IsDeadOrEscaped(target)))
            {
            ...
            {
                PCLGameEffects.List.Add(new ThoughtBubble(player.dialogX, player.dialogY, 3, card.cantUseMessage, true));
            }
