...
public class AnimatorRelicSlotSelectionEffect extends EYBEffect
{
    ...
    GUI_TextBox(GR.Common.Images.Panel_Rounded_Half_H.Texture(), new Hitbox(AbstractCard.IMG_WIDTH * 0.15f, AbstractCard.IMG_HEIGHT * 0.15f))
    ...
    private final AnimatorRelicSlot slot;
    private final ArrayList<RenderItem> relics = new ArrayList<>();
    //private boolean draggingScreen = false;
    private EYBRelic selectedRelic;

    public AnimatorRelicSlotSelectionEffect(AnimatorRelicSlot slot)
    {
        super(0.7f, true);
        ...
        this.slot = slot;
        final ArrayList<AnimatorRelicSlot.Item> slotItems = slot.GetSelectableRelics();
    ...
    private void OnRelicClicked(EYBRelic relic)
    {
        if (selectedRelic != null)
        {
    ...
    public class RenderItem
    {
        ...
        public RenderItem(AnimatorRelicSlot.Item item, int index)
        {
