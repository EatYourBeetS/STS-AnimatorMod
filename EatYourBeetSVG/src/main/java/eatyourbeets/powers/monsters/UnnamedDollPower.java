...
import eatyourbeets.powers.common.DelayedStrengthPower;
        ...
        GameActions.Last.Callback(() ->
        {
            if (!AbstractDungeon.getCurrRoom().isBattleEnding())
            {
                for (AbstractCreature c : GameUtilities.GetAllCharacters(true))
                {
                    ...
                    if (GameUtilities.IsPlayerTurn(true) || c.isPlayer)
                    {
                        GameActions.Bottom.StackPower(null, new StrengthPower(c, STRENGTH));
                    }
                    else
                    {
                        GameActions.Bottom.StackPower(null, new DelayedStrengthPower(c, STRENGTH));
                    }
                }
            }
        });
