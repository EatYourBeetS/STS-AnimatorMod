...
public class TheEgnaroPiece extends UnnamedReignRelic
{
    ...
    public static final int EMPOWER_AMOUNT = 1;
    ...
    {
        return JUtils.Format(DESCRIPTIONS[0], EMPOWER_AMOUNT, CARDS_STEP);
    }
    ...
    {
        this.counter = 1 + Math.min(9, EMPOWER_AMOUNT * player.masterDeck.size() / CARDS_STEP);
        if (counter > 0)
        {
            GameActions.Bottom.GainEmpower(counter);
            flash();
        }
    }
