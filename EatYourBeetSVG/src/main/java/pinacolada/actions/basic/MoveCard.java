...
public class MoveCard extends EYBActionWithCallback<AbstractCard>
{
    ...
    protected void FirstUpdate()
    {
        if (sourcePile == null)
        {
            sourcePile = PCLGameUtilities.FindCardGroup(card, false);

            if (sourcePile == null)
            {
                PCLJUtils.LogWarning(this, "Could not find card source pile.");
                Complete();
                return;
            }
        }
        ...
        {
            PCLJUtils.LogWarning(this, "Could not find " + card.cardID + " in " + sourcePile.type.name().toLowerCase());
            Complete();
            return;
        }

        if (PCLGameUtilities.TrySetPosition(sourcePile, card) && showEffect)
        {
            PCLGameEffects.TopLevelList.Add(new RenderCardEffect(card, duration, isRealtime));
        }
    ...
    protected void UpdateInternal(float deltaTime)
    {
        ...
        if (TickDuration(deltaTime))
        {
            ...
            {
                PCLGameEffects.Queue.Add(new UnfadeOutEffect(card));
                PCLActions.Bottom.Callback(() -> PCLGameEffects.Queue.Add(new UnfadeOutEffect(card)));
            }
    ...
    protected void Purge()
    {
        ...
        if (showEffect)
        {
            ...
            final Vector2 pos = PCLGameUtilities.TryGetPosition(sourcePile, card);
            final AbstractGameEffect effect = PCLGameEffects.List.Add(new PurgeCardEffect(card, pos.x, pos.y));
        ...
        PCLCombatStats.OnPurge(card, sourcePile);
    }
