...
public class LoseHP extends EYBActionWithCallback<DamageInfo>
{
    protected DamageInfo damageInfo = null;
    ...
    protected void UpdateInternal(float deltaTime)
    {
        if (TickDuration(deltaTime))
        {
            ...
            if (!canKill)
            {
                amount = Math.max(0, Math.min(GameUtilities.GetHP(target, !ignoreTempHP, false) - 1, amount));
            }

            this.damageInfo = new DamageInfo(this.source, this.amount, DamageInfo.DamageType.HP_LOSS);
