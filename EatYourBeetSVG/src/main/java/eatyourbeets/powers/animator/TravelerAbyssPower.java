...
public class TravelerAbyssPower extends AnimatorPower {
    ...
    private enum AbyssNegativeEffect {
        ...
        EndTurn(ACTIONS.EndTurn(true), 4, (c, p, m) -> GameActions.Last.Add(new PressEndTurnButtonAction()));
    ...
    private enum AbyssPositiveEffect {
        ...
        ObtainGenshinCard(ACTIONS.AddRandomMotivatedCard(CardSeries.GenshinImpact.Name, true), 6, 3, (c, p, m) -> {
            AbstractCard gCard = genshinCards.Retrieve(rng, false);
            GameActions.Bottom.MakeCardInDrawPile(gCard);
        }),
        ObtainWish(ACTIONS.AddToDrawPile(1, Traveler_Wish.DATA.Strings.NAME, true), 6, 3, (c, p, m) -> GameActions.Bottom.MakeCardInDrawPile(new Traveler_Wish())),
        PlayDainsleif(ACTIONS.PlayFromAnywhere(Dainsleif.DATA.Strings.NAME, true), 5, 4, (c, p, m) -> GameActions.Bottom.PlayFromPile(null, 1, m, p.drawPile, p.discardPile, p.hand).SetOptions(true, false).SetFilter(ca -> ca instanceof Dainsleif));
