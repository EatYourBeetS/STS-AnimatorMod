...
public class ApplyPower extends EYBActionWithCallback<AbstractPower>
{
    ...
    protected void FirstUpdate()
    {
        ...
        {
            target = PCLGameUtilities.GetRandomEnemy(true);
        ...
        if (shouldCancelAction() || HardCodedStuff_NoDraw() || !PCLGameUtilities.CanApplyPower(source, target, powerToApply, this))
        {
        ...
        if (powerToApply instanceof AbstractPCLAffinityPower)
        {
            AbstractPCLAffinityPower attributePower = (AbstractPCLAffinityPower) powerToApply;
    ...
    private void StackPower(AbstractPower power)
    {
        ...
        if (showEffect)
        {
            ...
            {
                PCLGameEffects.List.Add(new PowerDebuffEffect(target.hb.cX - target.animX,
            ...
            else if (amount > 0)
            {
                ...
                {
                    PCLGameEffects.List.Add(new PowerBuffEffect(target.hb.cX - target.animX,
                    target.hb.cY + target.hb.height / 2f, "+" + amount + " " + powerToApply.name));
                }
            }
    ...
    private void AddPower()
    {
        ...
        if (showEffect)
        {
            ...
            {
                PCLGameEffects.List.Add(new PowerBuffEffect(target.hb.cX - target.animX, target.hb.cY + target.hb.height / 2f, powerToApply.name));
            }
            else
            {
                PCLGameEffects.List.Add(new PowerDebuffEffect(target.hb.cX - target.animX, target.hb.cY + target.hb.height / 2f, powerToApply.name));
            }
        }
