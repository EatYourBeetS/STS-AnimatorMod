...
public class MeteorFallEffect extends AbstractGameEffect
{
    ...
    public void update()
    {
        if (duration == DUR)
        {
            for (int i = 0; i < 50; ++i)
            {
                PCLGameEffects.Queue.Add(new GenericSmokeEffect(x + MathUtils.random(-280f, 250f) * Settings.scale, y - 80f * Settings.scale));
            }
        }
        ...
        if (this.duration < 0f)
        {
            ...
            {
                PCLGameEffects.Queue.Add(new DarkSmokePuffEffect(this.x, this.y));

                for (int i = 0; i < 12; ++i)
                {
                    PCLGameEffects.Queue.Add(new SmokingEmberEffect(this.x + MathUtils.random(-50f, 50f) * Settings.scale, this.y + MathUtils.random(-50f, 50f) * Settings.scale));
                }
            }
