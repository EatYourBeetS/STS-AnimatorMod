...
public class GenerateCard extends EYBActionWithCallback<AbstractCard>
{
    ...
    protected void FirstUpdate()
    {
        ...
        switch (cardGroup.type)
        {
            case DRAW_PILE:
            {
                effect = PCLGameEffects.List.Add(new ShowCardAndAddToDrawPileEffect(actualCard,
                ...
                actualCard = PCLJUtils.<AbstractCard>GetField("card", ShowCardAndAddToDrawPileEffect.class).Get(effect);
            ...
            case HAND:
            {
                if (player.hand.size() >= BaseMod.MAX_HAND_SIZE)
                {
                    ...
                    effect = PCLGameEffects.List.Add(new ShowCardAndAddToDiscardEffect(actualCard));
                }
                else
                {
                    // If you don't specify x and y it won't play the card obtain sfx
                    effect = PCLGameEffects.List.Add(new ShowCardAndAddToHandEffect(actualCard,
                    (float) Settings.WIDTH / 2f - ((25f * Settings.scale) + AbstractCard.IMG_WIDTH),
            ...
            case EXHAUST_PILE:
            {
                effect = PCLGameEffects.List.Add(new ExhaustCardEffect(actualCard));
            ...
            case MASTER_DECK:
            {
                ...
                if (destination != null)
                {
                    PCLJUtils.LogWarning(this, "Destination for the master deck will be ignored.");
                    destination = null;
                }
            ...
            default:
            {
                PCLJUtils.LogWarning(this, "Can't make temp card in " + cardGroup.type.name());
                Complete();
                break;
            }
        }
