...
public class ReduceStrength extends EYBActionWithCallback<Boolean>
{
    ...
    protected void FirstUpdate()
    {
        ...
        if (temporary)
        {
            PCLActions.Top.StackPower(source, new ShacklesPower(target, amount));
        }
        else
        {
            PCLActions.Top.StackPower(source, new StrengthPower(target, -amount));
        }
        ...
        if (!target.hasPower(ArtifactPower.POWER_ID))
        {
            ...
            {
                PCLActions.Top.GainMight(amount);
            }
            if (giveStrengthToSource)
            {
                PCLActions.Top.StackPower(new StrengthPower(source, amount));
            }
