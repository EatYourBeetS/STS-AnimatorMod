...
public class CreatureExportData implements Comparable<CreatureExportData>
{
    ...
    private void exportImageToFile(String imageFile)
    {
        ...
        float x_padding = 90.0f;
        float y_padding = 40.0f;
        float x = creature.hb.x - x_padding;
        float y = creature.hb.y - y_padding;
        float width = creature.hb.width + 2 * x_padding;
        float height = creature.hb.height + 2 * y_padding;
    ...
    public static ArrayList<AbstractPlayer> getAllPlayers()
    {
        ...
        try
        {
            ...
            for (AbstractPlayer.PlayerClass playerClass : AbstractPlayer.PlayerClass.values())
            {
                ...
                {
                    Exporter.logger.error("Exception occurred when creating character", e);
                }
            }
        }
        catch (Exception e)
        {
            Exporter.logger.error("Exception occurred when getting createCharacter method", e);
        }
        return players;
    }
    ...
    public static ArrayList<AbstractMonster> getAllMonsters()
    {
        ...
        HashMap<String, BaseMod.GetMonsterGroup> customMonsters = ReflectionHacks.getPrivateStatic(BaseMod.class, "customMonsters");
        ...
        creatures.add(byrd);
        creatures.sort(Comparator.comparing((AbstractMonster a) -> a.name));
        return creatures;
    }
    ...
    {
        if (creature.isPlayer && !that.creature.isPlayer)
        {
            return -1;
        }
        if (!creature.isPlayer && that.creature.isPlayer)
