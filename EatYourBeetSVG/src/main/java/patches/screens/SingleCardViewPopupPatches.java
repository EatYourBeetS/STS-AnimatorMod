...
public class SingleCardViewPopupPatches
{
    ...
    public static class SingleCardViewPopup_RenderDescription
    {
        @SpireInsertPatch(rloc = 0, localvars = {"card"})
        public static SpireReturn Insert(SingleCardViewPopup __instance, SpriteBatch sb, AbstractCard card)
        {
            EYBCard c = JavaUtilities.SafeCast(card, EYBCard.class);
            if (c != null && !c.isFlipped)
            {
                c.renderDescription(sb);
