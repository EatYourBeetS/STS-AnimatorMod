...
public class Mathf
{
    ...
    public static int Sign(int i) { return i > 0 ? 1 : -1; }
    ...
    static public int MoveTowards(int current, int target, int progress)
    {
    ...
    public static float GetOffsetFromCenter(int index, int max)
    {
        //noinspection IntegerDivisionInFloatingPointContext
        return (index - (max / 2)) + (max % 2 == 1 ? 0 : 0.5f);
    }
    ...
    public static int[] Range(int a, int b, int step)
    {
        ...
        final int length = Abs(a - b);
        final int size = (length / step) + (length % step > 0 ? 1 : 0);
        final int[] res = new int[size];
        if (a > b)
        {
            for (int i = 0; i < size; i++)
            {
                res[i] = Max(b, a - (step * i));
            }
        }
        else
        {
