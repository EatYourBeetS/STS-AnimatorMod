...
{
    protected static final HashMap<AttackEffect, AttackEffectData> map = new HashMap<>();
    protected static final CommonImages.Effects IMAGES = GR.Common.Images.Effects;
    protected static final ArrayList<AttackEffect> melee = new ArrayList<>();
    protected static final ArrayList<AttackEffect> magic = new ArrayList<>();
    protected static final ArrayList<AttackEffect> other = new ArrayList<>();
...
//   public static final AttackEffect PSYCHOKINESIS = GR.Enums.AttackEffect.PSYCHOKINESIS;
    ...
    public static void PlaySound(AttackEffect effect, float pitchMin, float pitchMax, float volume)
    {
        SFX.Play(GetSoundKey(effect), pitchMin, pitchMax, volume);
    }
...
//              .SetDamageTint(Color.PINK);
        ...
        Add(melee, CLAW)
                .SetVFX((cX, cY) -> VFX.Claw(cX, cY, Color.VIOLET, Color.WHITE).PlaySFX(false))
                .SetSFX(SFX.ATTACK_DAGGER_5, SFX.ATTACK_DAGGER_6);

        Add(melee, BITE)
                .SetVFX((cX, cY) -> VFX.Bite(cX, cY).PlaySFX(false))
...
//              .SetDamageTint(Color.BLUE);
    ...
    protected static class AttackEffectData
    {
        ...
        protected FuncT2<EYBEffect, Float, Float> createVFX;
        protected FuncT3<EYBEffect, AbstractCreature, Float, Float> createVFX2;
        protected TextureRegion texture;
        protected float damageDelay;
        protected Color damageTint;
