...
public class AnimatorLoadoutEditor extends AbstractScreen
{
    ...
    protected AnimatorBaseStatEditor buffEditor;
    ...
    protected final float label_cy_1;
    protected final float label_cy_2;
    ...
    protected GUI_TextBox deckSizeValue_text;
    ...
    public AnimatorLoadoutEditor()
    {
        ...
        for (int i = 0; i < preset_buttons.length; i++)
        {
            ...
            if (i > 0)
            {
                preset_buttons[i].SetTooltip("Locked", GR.Animator.Strings.CharSelect.ObtainBronzeAtAscension(i == 1
                        ...
                        : AnimatorLoadout.BRONZE_REQUIRED_PRESET_SLOT_3), false);
            }
        }
        ...
        .SetColors(Settings.HALF_TRANSPARENT_BLACK_COLOR, Settings.CREAM_COLOR)
        .SetAlignment(0.5f, 0.5f)
        .SetPosition(save_button.hb.cX, label_cy_1 = (cardsCount_text.hb.y + cardsCount_text.hb.height + labelHeight * 0.8f))
        .SetFont(FontHelper.charDescFont, 1);

        deckSizeValue_text = new GUI_TextBox(GR.Common.Images.Panel_Rounded.Texture(), new Hitbox(labelWidth, labelHeight))
        .SetColors(Settings.HALF_TRANSPARENT_BLACK_COLOR, Settings.CREAM_COLOR)
        .SetAlignment(0.5f, 0.5f)
        .SetPosition(save_button.hb.cX, label_cy_2 = (affinityValue_text.hb.y + affinityValue_text.hb.height + labelHeight * 0.8f))
        .SetFont(FontHelper.charDescFont, 1);
        ...
        hpEditor = new AnimatorBaseStatEditor.HP(ScreenW(0.666f), ScreenH(0.442f));
        goldEditor = new AnimatorBaseStatEditor.Gold(ScreenW(0.666f), ScreenH(0.353f));
        buffEditor = new AnimatorBaseStatEditor.Buff(ScreenW(0.666f), ScreenH(0.264f));

        ascensionRequirement = new GUI_TextBox(GR.Common.Images.Panel_Rounded.Texture(), new Hitbox(labelWidth, labelHeight * 6))
        ...
        .SetAlignment(0.5f, 0.5f)
        .SetPosition(ScreenW(0.666f), ScreenH(0.325f))
    ...
    public void Open(AnimatorLoadout loadout, CharacterOption option, ActionT0 onClose)
    {
        ...
        buffEditor.SetInteractable(enableHPAndGoldEditor);
    ...
    public void Update()
    {
        ...
        else
        {
            ...
            buffEditor.SetEstimatedValue(val.BuffValue).Update();
        ...
        deckSizeValue_text.SetText("Deck Size: -" + val.DeckSizeBonus).SetActive(val.DeckSizeBonus > 0).TryUpdate();
        affinityValue_text.SetText("Affinity: +" + val.AffinityLevel).SetActive(val.AffinityLevel > 0).TryUpdate();

        if (affinityValue_text.isActive)
        {
            deckSizeValue_text.SetPosition(null, label_cy_2);
            affinityValue_text.SetPosition(null, label_cy_1);
        }
        else if (deckSizeValue_text.isActive)
        {
            deckSizeValue_text.SetPosition(null, label_cy_1);
            affinityValue_text.SetPosition(null, label_cy_2);
        }
    ...
    public void Render(SpriteBatch sb)
    {
        ...
        deckSizeValue_text.TryRender(sb);
    ...
    public void ChangePreset(int preset)
    {
        ...
        this.buffEditor.SetLoadout(presets[preset]);
        SetSlotsActive(true);
    }
