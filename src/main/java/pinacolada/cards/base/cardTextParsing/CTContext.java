...
public class CTContext
{
    ...
    public void Initialize(PCLCard card, String text)
    {
        ...
        if (card != null)
        {
            ...
            if (text.length() > 2 && Character.isDigit(text.charAt(0)))
            {
                ...
                if (i > 0)
                {
                    this.scaleModifier = PCLJUtils.ParseInt(text.substring(0, i), 100) / 100f;
        ...
        while (MoveIndex(amount))
        {
            ...
            {
                PCLJUtils.LogError(this, "Error parsing card text, Character: " + character + ", Text: " + this.text);
                amount = 1;
            }
        }
    ...
    protected void AddTooltip(PCLCardTooltip tooltip)
    {
